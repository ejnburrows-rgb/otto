        ...
        for i in range(5):
            t = page.evaluate('''async () => {
                ...
                for (const a of atts) {
                    ...
                    links.push(url ? `<a class="btn ghost sm" href="${url}" download="${esc(a.name)}"><i class="fas fa-paperclip"></i> ${esc(a.name)}</a>` : `<span class="pill gray"><i class="fas fa-paperclip"></i> ${esc(a.name)}</span>`);
                }
